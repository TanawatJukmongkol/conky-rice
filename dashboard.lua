...
canvas.draw = function ()
	...
	font("Anonymice Powerline", 11.8, true, true)
	-- Network speed --
	...
	speed_down = data.network:get("percentSpeed").down
		font("Hack", 10.5, true, true)
		colorHex(0x68b0c0, 1)
		stroke_width(2)
		text_center_center(data.network.cache.interface, img.x + img.w / 2 + 70, img.y + 92)
		-- upload speed
		bar_graph(img.x + img.w - 52, img.y + 42, img.x + img.w - 14, img.y + 58, speed_up, 0xa0a0a0, 0xffff00, 0.76)
		colorHex(0xffff00, speed_up == 0 and 0.42 or 0.76)
		text_center_center("▲ " .. data.network:get("speed").up .. "KiB", img.x + img.w / 2 + 61, img.y + 67)
		-- download speed
		bar_graph(img.x + img.w - 56, img.y + 120, img.x + img.w - 14, img.y + 112, speed_down, 0xa0a0a0, 0xaaff00, 0.76)
		colorHex(0xaaff00, speed_down == 0 and 0.42 or 0.76)
		text_center_center("▼ " .. data.network:get("speed").down .. "KiB", img.x + img.w / 2 + 68, img.y + 78)
	-- CPU --
	data.cpu:update()
	font("Anonymice Powerline", 11.8, true, true)
	...
	text("FAN: " .. data.sensors:get({"asus-isa-0000", "cpu_fan", "fan1_input"}) .. " RPM", img.x + img.w - 67, img.y + img.h - 60)
	colorHex(0xaaccff, 0.69)
	text("NVME: " .. data.sensors:get({"nvme-pci-0400", "Composite", "temp1_input"}) .. " °C", img.x + img.w - 72, img.y + img.h - 50)
	colorHex(0xaaffcc, 0.69)
	text("APCI: " .. data.sensors:get({"acpitz-acpi-0", "temp1", "temp1_input"}) .. " °C", img.x + img.w - 77, img.y + img.h - 40)
	colorHex(0xaaff00, 0.69)
	text("PCIe: " .. data.sensors:get({"k10temp-pci-00c3", "Tctl", "temp1_input"}) .. " °C", img.x + img.w - 82, img.y + img.h - 30)
	colorHex(0xffff00, 0.69)
	text("Battery: " .. data.sensors:get({"BAT1-acpi-0", "in0", "in0_input"}) .. " V", img.x + img.w - 77, img.y + img.h - 20)
	...
	text("Power draw: " .. string.format("%g",string.format("%.1f", power_draw)) .. " W", img.x + img.w - 72, img.y + img.h - 10)
	colorHex(0xffaa00, 0.69)
	text(string.format("%.2f", data.sensors:get({"BAT1-acpi-0", "curr1", "curr1_input"})) .. " A", img.x + img.w - 4, img.y + img.h)
